...
- (int)createBootstrapFile {
    ...
    if (NO == fileExisted) {
        ...
        @"This file is used to automatically run `s7 init` when an existing s7 repo is cloned.\n"
         ...
         " 0. on istallation, s7 registers as the filter in global git config.\n"
         ...
         "\n"
         " 2. once `git clone` is complete, Git checks out files into working tree.\n"
         "    .s7bootstrap is also checked out and s7 filter is called.\n"
         "    This is the backdoor we will use to call `s7 init` automatically.\n"
         "\n"
         " 3. s7 filter cannot call `s7 init` right away as there's no guarantie that other\n"
         ...
         "    This is theoretically possible if user ran cloned with custom templates\n"
         ...
         "    not the situation we want user to deal with.\n"
         ;
